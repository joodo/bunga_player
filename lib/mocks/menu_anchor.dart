...
/// The type of builder function used by [MyMenuAnchor.builder] to build the
/// widget that the [MyMenuAnchor] surrounds.
///
...
/// the menu with.
///
/// The `child` is an optional child supplied as the [MyMenuAnchor.child]
...
///
/// When creating a menu with [MenuBar] or a [SubmenuButton], a [MyMenuAnchor] is
/// not needed, since they provide their own internally.
///
/// The [MyMenuAnchor] is meant to be a slightly lower level interface than
/// [MenuBar], used in situations where a [MenuBar] isn't appropriate, or to
/// construct widgets or screen regions that have submenus.
///
/// {@tool dartpad}
/// This example shows how to use a [MyMenuAnchor] to wrap a button and open a
...
///
/// {@tool dartpad}
/// This example shows how to use a [MyMenuAnchor] to create a cascading context
...
class _MyMenuAnchorState extends State<MyMenuAnchor> {
  ...
  @override
  void didUpdateWidget(MyMenuAnchor oldWidget) {
  ...
  void _addChild(_MyMenuAnchorState child) {
  ...
  _MyMenuAnchorState get _root {
    _MyMenuAnchorState anchor = this;
  ...
  /// the local coordinates of the [MyMenuAnchor], ignoring any
  /// [MenuStyle.alignment] and/or [MyMenuAnchor.alignmentOffset] that were
  /// specified.
  ...
  void _closeChildren({bool inDispose = false}) {
    ...
    for (final _MyMenuAnchorState child
        in List<_MyMenuAnchorState>.from(_anchorChildren)) {
  ...
  // changes.
  static _MyMenuAnchorState? _maybeOf(BuildContext context) {
    return context
...
/// A controller to manage a menu created by a [MenuBar] or [MyMenuAnchor].
///
...
/// See also:
///
/// * [MyMenuAnchor], a widget that defines a region that has submenu.
...
class MenuController {
  ...
  /// [MyMenuAnchor]. A [MenuController] is also be received by the
  /// [MyMenuAnchor.builder] when invoked.
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [MyMenuAnchor]) is
  ...
  /// the coordinate space of the [MyMenuAnchor] this controller is attached to.
  ///
  /// If given, the `position` will override the [MyMenuAnchor.alignmentOffset]
  /// given to the [MyMenuAnchor].
  ///
  /// If the menu's anchor point (either a [MenuBar] or a [MyMenuAnchor]) is
  /// scrolled by an ancestor, or the view changes size, then any open menu will
  /// automatically close.
  ...
  void _attach(_MyMenuAnchorState anchor) {
    _anchor = anchor;
  }

  void _detach(_MyMenuAnchorState anchor) {
    if (_anchor == anchor) {
      _anchor = null;
    }
  }
...
/// See also:
///
/// * [MyMenuAnchor], a widget that creates a region with a submenu and shows it
///   when requested.
...
/// A button for use in a [MenuBar], in a menu created with [MyMenuAnchor], or on
...
class _MenuItemButtonState extends State<MenuItemButton> {
  ...
  void _handleFocusChange() {
    if (!_focusNode.hasPrimaryFocus) {
      ...
      _MyMenuAnchorState._maybeOf(context)?._closeChildren();
    }
  }
  ...
  void _handleSelect() {
    ...
    if (widget.closeOnActivate) {
      _MyMenuAnchorState._maybeOf(context)?._root._close();
    }
...
/// - [MyMenuAnchor], a widget that defines a region which can host a cascading
///   menu.
...
class SubmenuButton extends StatefulWidget {
  ...
  /// top of the [MyMenuAnchor] region.
  final Offset? alignmentOffset;
...
class _SubmenuButtonState extends State<SubmenuButton> {
  ...
  MenuController get _menuController =>
      widget.controller ?? _internalMenuController!;
  _MyMenuAnchorState? get _anchor => _MyMenuAnchorState._maybeOf(context);
...
/// See also:
///
///  * [MyMenuAnchor], a widget that hosts a cascading submenu.
...
class _MenuAnchorScope extends InheritedWidget {
  ...
  final _MyMenuAnchorState anchor;
  final bool isOpen;
...
/// MenuBar-specific private specialization of [MyMenuAnchor] so that it can act
/// differently in regards to orientation, how open works, and what gets built.
class _MenuBarAnchor extends MyMenuAnchor {
  ...
  @override
  State<MyMenuAnchor> createState() => _MenuBarAnchorState();
}

class _MenuBarAnchorState extends _MyMenuAnchorState {
  @override
  bool get _isOpen {
    // If it's a bar, then it's "open" if any of its children are open.
    for (final _MyMenuAnchorState child in _anchorChildren) {
      if (child._isOpen) {
        return true;
      }
    }
...
class _MenuDirectionalFocusAction extends DirectionalFocusAction {
  ...
  bool _moveToNext(_MyMenuAnchorState currentMenu) {
  ...
  bool _moveToParent(_MyMenuAnchorState currentMenu) {
  ...
  bool _moveToPrevious(_MyMenuAnchorState currentMenu) {
  ...
  bool _moveToSubmenu(_MyMenuAnchorState currentMenu) {
...
/// [MyMenuAnchor] or [MenuBar]. Provided menu items such as [MenuItemButton] and
...
class _MenuAcceleratorLabelState extends State<MenuAcceleratorLabel> {
  ...
  void didChangeDependencies() {
    ...
    _anchor = _MyMenuAnchorState._maybeOf(context);
