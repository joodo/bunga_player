...
class ScreenshotAction extends ContextAction<ScreenshotIntent> {
  ...
  Future<File> invoke(ScreenshotIntent intent, [BuildContext? context]) async {
    ...
    if (context != null && context.mounted) {
      context.read<PlayMessageEvent>().fire('已截图 $fileName');
...
extension PlayActionsExtension on Widget {
  Widget playActions({
    ...
    return Builder(
      builder: (context) => shortcuts.actions(
        actions: {
          ...
          OpenVideoIntent: OpenVideoAction(payloadNotifer: playPayloadNotifier),
          ...
          RefreshDirIntent: RefreshDirAction(dirInfoNotifier: dirInfoNotifier),
        },
      ),
    );
  }
