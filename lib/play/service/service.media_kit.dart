...
class MediaKitPlayService implements PlayService {
  ...
  SubtitleTrack setSubtitleTrack(String id) {
    ...
    final track = subtitleTracks.firstWhere(
      (track) => track.id == id || (track.title?.startsWith(',$id,') ?? false),
    );
    _player.setSubtitleTrack(track);
    return subtitleTracksNotifier.value.firstWhere(
...
Stream<bool> _bufferingStream(
  ...
  await for (final event in StreamGroup.merge([
    ...
    if (type == 'p') position = value;
    if (type == 'b') buffer = value;
